...
SQL queries for coverage.db (plans + claims tables).

## Sample member questions

These sample member questions are covered below:
...
4. JOIN claims and plans (show claim with plan details)
...
## Query 3 - monthly premium under 400
...
## Query 4 - JOIN between claims and plans
...
Equivalent form with INNER JOIN:

```sql
SELECT claims.claim_id, plans.plan_name
FROM claims
INNER JOIN plans ON claims.plan_id = plans.plan_id;
```
...
Keywords present: SELECT, FROM, JOIN, INNER JOIN, ON

---

## Query 5 - top-N procedures

Question: What are the most claimed procedures? (top-N)
...
- File: structured_queries.md
- Database: coverage.db
- Tables: plans, claims
- Includes SELECT queries
- Includes a JOIN between claims and plans
- Covers sample member questions listed above
